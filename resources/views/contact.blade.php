...
<head>
    ...
    <title>{{ __('Contact Us') }} - {{ __('site_title') }}</title>
            ...
            <div id="Container">
                        ...
                        <nav>
                            <div class="Menu">
                                ...
                                <ul>
                                    ...
                                    @endforeach
                                    <li><a href="{{ route('contact') }}" class="active">{{ __('Contact') }}</a></li>
                                </ul>
                            </div>
                        </nav>
                        <!--/Menu-->
                ...
                <div id="Body">
                    <div id="Content">
                        ...
                        <!-- Contact Methods Section (Replaces Our_Services) -->
                        <div id="Our_Services">
                            <div class="Our_Services">
                                <ul>
                                    @if($contactMethods->count() > 0)
                                        @foreach($contactMethods as $method)
                                            <li class="{{ $method->icon_class }}"> <a href="#"></a>
                                                <div class="img"></div>
                                                <h2>{{ $method->title }}</h2>
                                                <p>{{ $method->content }}</p>
                                            </li>
                                    ...
                                    @else
                                        <li>
                                            <p class="text-center" style="padding: 20px;">No contact information available.
                                            </p>
                                        </li>
                                    @endif
                                </ul>
                        ...
                        <div id="Host_Table" style="direction: rtl">
                            <div class="Table_top" style="direction: rtl">
                                <h2 class="right">{{ $hostingPlans['silver']->name ?? 'Silver' }} <span
                                        style="font-size:40px">{{ $hostingPlans['silver']->price ?? '200' }}</span><b>{{ $hostingPlans['silver']->currency ?? 'RS' }}</b>
                                </h2>
                                <h2 class="left">{{ $hostingPlans['gold']->name ?? 'Gold' }} <span
                                        style="font-size:40px">{{ $hostingPlans['gold']->price ?? '350' }}</span><b>{{ $hostingPlans['gold']->currency ?? 'RS' }}</b>
                                </h2>
                            </div>
                            <!--/top-->
                            <div class="Table_options">
                                <ul class="options_right">
                                    <li>{{ __('pricing.space') }}<span>{{ $hostingPlans['silver']->features['space'][app()->getLocale()] ?? $hostingPlans['silver']->features['space']['en'] ?? '10 GB' }}</span>
                                    </li>
                                    <li>{{ __('pricing.traffic') }}<span>{{ $hostingPlans['silver']->features['traffic'][app()->getLocale()] ?? $hostingPlans['silver']->features['traffic']['en'] ?? '100 GB' }}</span>
                                    </li>
                                    <li>{{ __('pricing.support_24') }}<span>{{ $hostingPlans['silver']->features['support'][app()->getLocale()] ?? $hostingPlans['silver']->features['support']['en'] ?? '24 Hours' }}</span>
                                    </li>
                                </ul>
                                <ul class="options_left">
                                    <li>{{ __('pricing.space') }}<span>{{ $hostingPlans['gold']->features['space'][app()->getLocale()] ?? $hostingPlans['gold']->features['space']['en'] ?? '10 GB' }}</span>
                                    </li>
                                    <li>{{ __('pricing.traffic') }}<span>{{ $hostingPlans['gold']->features['traffic'][app()->getLocale()] ?? $hostingPlans['gold']->features['traffic']['en'] ?? '100 GB' }}</span>
                                    </li>
                                    <li>{{ __('pricing.support_24') }}<span>{{ $hostingPlans['gold']->features['support'][app()->getLocale()] ?? $hostingPlans['gold']->features['support']['en'] ?? '24 Hours' }}</span>
                                    </li>
                                </ul>
                            </div>
                            <!--/options-->
                            <div class="Table_details">
                                <ul class="details_right">
                                    <li>{{ $hostingPlans['silver']->features['databases'][app()->getLocale()] ?? $hostingPlans['silver']->features['databases']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['silver']->features['subdomains'][app()->getLocale()] ?? $hostingPlans['silver']->features['subdomains']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['silver']->features['emails'][app()->getLocale()] ?? $hostingPlans['silver']->features['emails']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['silver']->features['ftp'][app()->getLocale()] ?? $hostingPlans['silver']->features['ftp']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li class="false">{{ __('pricing.custom_design') }}</li>
                                </ul>
                                <ul class="details_left">
                                    <li>{{ $hostingPlans['gold']->features['databases'][app()->getLocale()] ?? $hostingPlans['gold']->features['databases']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['gold']->features['subdomains'][app()->getLocale()] ?? $hostingPlans['gold']->features['subdomains']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['gold']->features['emails'][app()->getLocale()] ?? $hostingPlans['gold']->features['emails']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['gold']->features['ftp'][app()->getLocale()] ?? $hostingPlans['gold']->features['ftp']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li class="false">{{ __('pricing.custom_design') }}</li>
                                </ul>
                            </div>
                            ...
                            <div class="Host_Center">
                                <h2>{{ $hostingPlans['premium']->name ?? 'Premium' }} <span
                                        style="font-size:40px">{{ $hostingPlans['premium']->price ?? '400' }}</span><b>{{ $hostingPlans['premium']->currency ?? 'RS' }}</b>
                                </h2>
                                <ul class="options">
                                    <li>{{ __('pricing.space') }}<span>{{ $hostingPlans['premium']->features['space'][app()->getLocale()] ?? $hostingPlans['premium']->features['space']['en'] ?? '10 GB' }}</span>
                                    </li>
                                    <li>{{ __('pricing.traffic') }}<span>{{ $hostingPlans['premium']->features['traffic'][app()->getLocale()] ?? $hostingPlans['premium']->features['traffic']['en'] ?? '100 GB' }}</span>
                                    </li>
                                    <li>{{ __('pricing.support_24') }}<span>{{ $hostingPlans['premium']->features['support'][app()->getLocale()] ?? $hostingPlans['premium']->features['support']['en'] ?? '24 Hours' }}</span>
                                    </li>
                                    <li>{{ __('pricing.free_domain') }}
                                        <span>{{ $hostingPlans['premium']->features['free_domain'][app()->getLocale()] ?? $hostingPlans['premium']->features['free_domain']['en'] ?? 'Gift' }}</span>
                                    </li>
                                </ul>
                                <ul class="details">
                                    <li>{{ $hostingPlans['premium']->features['databases'][app()->getLocale()] ?? $hostingPlans['premium']->features['databases']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['premium']->features['subdomains'][app()->getLocale()] ?? $hostingPlans['premium']->features['subdomains']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['premium']->features['emails'][app()->getLocale()] ?? $hostingPlans['premium']->features['emails']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['premium']->features['ftp'][app()->getLocale()] ?? $hostingPlans['premium']->features['ftp']['en'] ?? 'Unlimited' }}
                                    </li>
                                    <li>{{ $hostingPlans['premium']->features['custom_design'][app()->getLocale()] ?? $hostingPlans['premium']->features['custom_design']['en'] ?? 'Custom Design' }}
                                    </li>
                                </ul>
